...
import numpy as np
import logging
from dataset import SoccerNetClips, OnlineSoccerNetClips, SoccerNetClipsTesting, OnlineSoccerNetFrames
...
def main(args):
    ...
    if not args.test_only:    
        if args.version == 2:
            dataset_Train = OnlineSoccerNetFrames(path_frames = '/data-local/data1-hdd/axesparraguera/SoccerNetFrames', 
                         ...
                         split=["train"], framerate=2, chunk_size=10, framestride = 4, store = True)
            ...
            dataset_Valid = OnlineSoccerNetFrames(path_frames = '/data-local/data1-hdd/axesparraguera/SoccerNetFrames', 
                         ...
                         path_store = '/data-local/data1-hdd/axesparraguera/SoccerNetFrames/trial',
                         features_audio = 'audio_embeddings_2fps.npy', 
                         split=["valid"], framerate=2, chunk_size=10, framestride = 4, store = True)
