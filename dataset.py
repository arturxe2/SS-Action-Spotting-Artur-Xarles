...
class OnlineSoccerNetFrames(Dataset):
    def __init__(self, path_frames = '/data-local/data1-ssd/axesparraguera/SoccerNetFrames', 
        ...
        self.game_featsA = list()
        self.game_labels = list()
        stride = self.chunk_size * self.framerate
        ...
        if store:
            ...
            for game in tqdm(self.listGames):
                ...
                for annotation in labels["annotations"]:
                    ...
                    if half == 1:
                        if self.chunk_size * self.framerate >= stride:
                            for i in range(self.chunk_size * self.framerate // stride):
                                label_half1[max(a - self.chunk_size * self.framerate // stride + 1 + i, 0)][0] = 0 # not BG anymore
                                label_half1[max(a - self.chunk_size * self.framerate // stride + 1 + i, 0)][label+1] = 1
                            #label_half1[max(a - self.chunk_size//stride + 1, 0) : (a + 1)][0] = 0 # not BG anymore
                            
                        else:
                            a2 = (frame - self.chunk_size * self.framerate) // stride
                            if a != a2:
                                label_half1[a][0] = 0
                                label_half1[a][label+1] = 1
                    ...
                    if half == 2:
                        if self.chunk_size * self.framerate >= stride:
                            for i in range(self.chunk_size * self.framerate // stride):
                                label_half2[max(a - self.chunk_size * self.framerate // stride + 1 + i, 0)][0] = 0 # not BG anymore
                                label_half2[max(a - self.chunk_size * self.framerate // stride + 1 + i, 0)][label+1] = 1
                            #label_half1[max(a - self.chunk_size//stride + 1, 0) : (a + 1)][0] = 0 # not BG anymore
                ...
                for i in range(feat_half1A.shape[0]):
                    ...
                    np.save(path + '/half1_chunk' + str(i) + '_labels.npy', label_half1[i, :])
                    self.path_list.append(path + '/half1_chunk' + str(i) + '_')
                    self.frames_path.append([os.path.join(self.path_frames, game, 'half1', 'frame ' + str(min(frames1, (i * 25 * stride // self.framerate) // 4 * 4 + j * self.framestride)) + '.jpg') for j in range(self.frames_clip)])
                ...
                for i in range(feat_half2A.shape[0]):
                    ...
                    np.save(path + '/half2_chunk' + str(i) + '_labels.npy', label_half2[i, :])
                    self.path_list.append(path + '/half2_chunk' + str(i) + '_')
                    self.frames_path.append([os.path.join(self.path_frames, game, 'half2', 'frame ' + str(min(frames2, (i * 25 * stride // self.framerate) // 4 * 4 + j * self.framestride)) + '.jpg') for j in range(self.frames_clip)])
...
class OnlineSoccerNetFramesAudio(Dataset):
    def __init__(self, path_frames = '/data-local/data1-ssd/axesparraguera/SoccerNetFrames', 
                path_audio = '/data-local/data1-ssd/axesparraguera/SoccerNetAudio',  
                path_labels = "/data-net/datasets/SoccerNetv2/ResNET_TF2",
                path_store = '/data-local/data1-ssd/axesparraguera/SoccerNetSamples',
                features_audio = 'audio', 
                split=["train"], audio_framerate = 100, framerate=2, chunk_size=5, framestride = 4, store = True):
        ...
        self.game_featsA = list()
        self.game_labels = list()
        audio_stride = self.chunk_size * self.audio_framerate
        stride = self.chunk_size * framerate
        ...
        if store:
            ...
            for game in tqdm(self.listGames):
                ...
                feat_half1A = np.load(os.path.join(path_audio, game, features_audio + "1.npy"))
                feat_half1A = feat_half1A.reshape(-1, feat_half1A.shape[-1])
                feat_half2A = np.load(os.path.join(path_audio, game, features_audio + '2.npy'))
                feat_half2A = feat_half2A.reshape(-1, feat_half2A.shape[-1])

                feat_half1A = np.transpose(feat_half1A)
                feat_half2A = np.transpose(feat_half2A)
                ...
                #Visual frames bigger than audio features
                if math.ceil(frames1 // 25) * self.audio_framerate > feat_half1A.shape[0]:
                    feat_half1A_aux = np.zeros((math.ceil(frames1 // 25) * self.audio_framerate, feat_half1A.shape[1]))
                    feat_half1A_aux[:feat_half1A.shape[0]] = feat_half1A
                    feat_half1A_aux[feat_half1A.shape[0]:] = feat_half1A[feat_half1A.shape[0]-1]
                    feat_half1A = feat_half1A_aux
                    
                if math.ceil(frames2 // 25) * self.audio_framerate > feat_half2A.shape[0]:
                    feat_half2A_aux = np.zeros((math.ceil(frames2 // 25) * self.audio_framerate, feat_half2A.shape[1]))
                    feat_half2A_aux[:feat_half2A.shape[0]] = feat_half2A
                    feat_half2A_aux[feat_half2A.shape[0]:] = feat_half2A[feat_half2A.shape[0]-1]
                    feat_half2A = feat_half2A_aux
                ...
                #More audio than frames
                if math.ceil(frames1 // 25) * self.audio_framerate < feat_half1A.shape[0]:
                    feat_half1A = feat_half1A[0:math.ceil(frames1 // 25) * self.audio_framerate, :]
                
                
                if math.ceil(frames2 // 25) * self.audio_framerate < feat_half2A.shape[0]:
                    feat_half2A = feat_half2A[0:math.ceil(frames2 // 25) * self.audio_framerate, :]

                #Generate clips from features

                feat_half1A = feats2clip(torch.from_numpy(feat_half1A), stride=audio_stride, clip_length=self.chunk_size*self.audio_framerate)
                audio_shape = feat_half1A.shape 
                feat_half1A = feat_half1A.view(audio_shape[0], self.framerate * self.chunk_size, -1, audio_shape[2])

                feat_half2A = feats2clip(torch.from_numpy(feat_half2A), stride=audio_stride, clip_length=self.chunk_size*self.audio_framerate) 
                audio_shape = feat_half2A.shape
                feat_half2A = feat_half2A.view(audio_shape[0], self.framerate * self.chunk_size, -1, audio_shape[2])
                ...
                for i in range(feat_half1A.shape[0]):
                    np.save(path + '/half1_chunk' + str(i) + '_featuresA2.npy', feat_half1A[i, :, :])
                    np.save(path + '/half1_chunk' + str(i) + '_labels2.npy', label_half1[i, :])
                    self.path_list.append(path + '/half1_chunk' + str(i) + '_')
                    self.frames_path.append([os.path.join(self.path_frames, game, 'half1', 'frame ' + str(min(frames1, (i * 25 * stride // self.framerate) // 4 * 4 + j * self.framestride)) + '.jpg') for j in range(self.frames_clip)])
                    #self.path_list2.append(os.path.join(self.path_frames, game, 'half1', 'frame ' ))
                ...
                for i in range(feat_half2A.shape[0]):
                    np.save(path + '/half2_chunk' + str(i) + '_featuresA2.npy', feat_half2A[i, :, :])
                    np.save(path + '/half2_chunk' + str(i) + '_labels2.npy', label_half2[i, :])
                    self.path_list.append(path + '/half2_chunk' + str(i) + '_')
                    self.frames_path.append([os.path.join(self.path_frames, game, 'half2', 'frame ' + str(min(frames2, (i * 25 * stride // self.framerate) // 4 * 4 + j * self.framestride)) + '.jpg') for j in range(self.frames_clip)])
                    #self.path_list2.append(os.path.join(self.path_frames, game, 'half2', 'frame ' ))
            ...
            #Concatenate features
            with open(self.path_store + '/' + split[0] + '_chunk_list.pkl', 'wb') as f:
                pickle.dump(self.path_list, f)
            with open(self.path_store + '/' + split[0] + '_frames_path.pkl', 'wb') as f:
                pickle.dump(self.frames_path, f)

                
        else:
            with open(self.path_store + '/' + split[0] + '_chunk_list.pkl', 'rb') as f:
                self.path_list = pickle.load(f)
            with open(self.path_store + '/' + split[0] + '_frames_path.pkl', 'rb') as f:
                self.frames_path = pickle.load(f)
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(np.array(frames)), torch.from_numpy(np.load(path + 'featuresA2.npy')), np.load(path + 'labels2.npy')
...
class SoccerNetFramesTesting(Dataset):
    ...
    def __init__(self, path_frames = '/data-local/data1-ssd/axesparraguera/SoccerNetFrames', 
                ...
                path_labels = "/data-net/datasets/SoccerNetv2/ResNET_TF2",
                features_audio = 'audio_embeddings_2fps.npy', 
                split=["test"], framerate=2, chunk_size=20, framestride = 8):
    ...
    def __getitem__(self, index):
        ...
        label_half1 = np.zeros((featA_half1.shape[0], self.num_classes))
        label_half2 = np.zeros((featA_half2.shape[0], self.num_classes))
        ...
        if os.path.exists(os.path.join(self.path_labels, self.listGames[index], self.labels)):
            ...
            for annotation in labels["annotations"]:
                ...
                if half == 1:
                    frame = min(frame, featA_half1.shape[0]-1)
                    label_half1[frame][label] = value
    
                if half == 2:
                    frame = min(frame, featA_half2.shape[0]-1)
                    label_half2[frame][label] = value
        ...
        #Visual frames bigger than audio features
        if math.ceil(frames1 // 25) * 2 > featA_half1.shape[0]:
            feat_half1A_aux = np.zeros((math.ceil(frames1 // 25) * 2, featA_half1.shape[1]))
            feat_half1A_aux[:featA_half1.shape[0]] = featA_half1
            feat_half1A_aux[featA_half1.shape[0]:] = featA_half1[featA_half1.shape[0]-1]
            featA_half1 = feat_half1A_aux
                    
        if math.ceil(frames2 // 25) * 2 > featA_half2.shape[0]:
            feat_half2A_aux = np.zeros((math.ceil(frames2 // 25) * 2, featA_half2.shape[1]))
            feat_half2A_aux[:featA_half2.shape[0]] = featA_half2
            feat_half2A_aux[featA_half2.shape[0]:] = featA_half2[featA_half2.shape[0]-1]
            featA_half2 = feat_half2A_aux
                
        #More audio than frames
        if math.ceil(frames1 // 25) * 2 < featA_half1.shape[0]:
            featA_half1 = featA_half1[0:math.ceil(frames1 // 25) * 2, :]
                        
        if math.ceil(frames2 // 25) * 2 < featA_half2.shape[0]:
            featA_half2 = featA_half2[0:math.ceil(frames2 // 25) * 2, :]
        ...
        return self.listGames[index], featA_half1, featA_half2, label_half1, label_half2, frames1, frames2
...
#Class to generate the samples for the test part
class SoccerNetFramesAudioTesting(Dataset):
    ...
    def __init__(self, path_frames = '/data-local/data1-ssd/axesparraguera/SoccerNetFrames', 
                ...
                path_labels = "/data-net/datasets/SoccerNetv2/ResNET_TF2",
                features_audio = 'audio', 
                split=["test"], audio_framerate = 100, framerate=2, chunk_size=4, framestride = 8, stride = 0.5):
        ...
        self.framestride = framestride
        self.split = split
        self.audio_stride = stride * audio_framerate
        self.stride = stride * framerate
    ...
    def __getitem__(self, index):
        ...
        featA_half1 = np.load(os.path.join(self.path_audio, self.listGames[index], self.features_audio + '1.npy'))
        featA_half1 = featA_half1.reshape(-1, featA_half1.shape[-1])    #for C3D non PCA
        
        featA_half2 = np.load(os.path.join(self.path_audio, self.listGames[index], self.features_audio + '2.npy'))
        featA_half2 = featA_half2.reshape(-1, featA_half2.shape[-1])    #for C3D non PCA

        featA_half1 = np.transpose(featA_half1)
        featA_half2 = np.transpose(featA_half2)


            
        label_half1 = np.zeros((int(featA_half1.shape[0] // self.audio_stride), self.num_classes))
        label_half2 = np.zeros((int(featA_half2.shape[0] // self.audio_stride), self.num_classes))
        ...
        if os.path.exists(os.path.join(self.path_labels, self.listGames[index], self.labels)):
            ...
            for annotation in labels["annotations"]:
                ...
                minutes = int(time[-5:-3])
                seconds = int(time[-2::])
                frame = int(self.framerate * ( seconds + 60 * minutes ) // self.stride)
                ...
                if half == 1:
                    frame = min(frame, label_half1.shape[0]-1)
                    label_half1[frame][label] = value
    
                if half == 2:
                    frame = min(frame, label_half2.shape[0]-1)
                    label_half2[frame][label] = value
        ...
        #Visual frames bigger than audio features
        if math.ceil(frames1 // 25) * self.audio_framerate > featA_half1.shape[0]:
            feat_half1A_aux = np.zeros((math.ceil(frames1 // 25) * self.audio_framerate, featA_half1.shape[1]))
            feat_half1A_aux[:featA_half1.shape[0]] = featA_half1
            feat_half1A_aux[featA_half1.shape[0]:] = featA_half1[featA_half1.shape[0]-1]
            featA_half1 = feat_half1A_aux
                    
        if math.ceil(frames2 // 25) * self.audio_framerate > featA_half2.shape[0]:
            feat_half2A_aux = np.zeros((math.ceil(frames2 // 25) * self.audio_framerate, featA_half2.shape[1]))
            feat_half2A_aux[:featA_half2.shape[0]] = featA_half2
            feat_half2A_aux[featA_half2.shape[0]:] = featA_half2[featA_half2.shape[0]-1]
            featA_half2 = feat_half2A_aux
        ...
        #More audio than frames
        if math.ceil(frames1 // 25) * self.audio_framerate < featA_half1.shape[0]:
            featA_half1 = featA_half1[0:math.ceil(frames1 // 25) * self.audio_framerate, :]
                        
        if math.ceil(frames2 // 25) * self.audio_framerate < featA_half2.shape[0]:
            featA_half2 = featA_half2[0:math.ceil(frames2 // 25) * self.audio_framerate, :]

        #Generate audio clips
            
        featA_half1 = feats2clip(torch.from_numpy(featA_half1),
                                    stride=int(self.audio_stride), off=int(self.chunk_size * self.audio_framerate / 2),
                                    clip_length=(self.chunk_size * self.audio_framerate))
        audio_shape = featA_half1.shape 
        featA_half1 = featA_half1.view(audio_shape[0], self.framerate * self.chunk_size, -1, audio_shape[2])
        featA_half2 = feats2clip(torch.from_numpy(featA_half2),
                                    stride=int(self.audio_stride), off=int(self.chunk_size * self.audio_framerate / 2),
                                    clip_length=self.chunk_size * self.audio_framerate)
        audio_shape = featA_half2.shape 
        featA_half2 = featA_half2.view(audio_shape[0], self.framerate * self.chunk_size, -1, audio_shape[2])
...
dataset_Valid = OnlineSoccerNetFramesAudio(path_frames = '/data-local/data1-ssd/axesparraguera/SoccerNetFrames', 
                ...
                path_store = '/data-local/data1-ssd/axesparraguera/SoccerNetSamples',
                features_audio = 'audio', 
                split=["valid"], audio_framerate = 100, framerate=2, chunk_size=5, framestride = 4, store = True)


print(dataset_Valid.__getitem__(0)[3][0:50, :])
