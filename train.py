...
def trainSS(dataloader,
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader), ncols=160) as t:
        ...
        for i, (featsV, featsA, labels) in t:
            ...
            loss = criterionMask(Vreal, Vpreds) + criterionMask(Areal, Apreds)# + criterionVA(classV, classA)
...
def testSpotting(dataloader, model, model_name, overwrite=True, NMS_window=30, NMS_threshold=0.5):
    ...
    if not os.path.exists(output_results) or overwrite:
        ...
        with tqdm(enumerate(dataloader), total=len(dataloader), ncols=120) as t:
            ...
            for i, (game_ID, featV_half1, featA_half1, featV_half2, featA_half2, label_half1, label_half2) in t:
                ...
                game_ID = game_ID[0]
                featV_half1 = featV_half1.squeeze(0)
                featA_half1 = featA_half1.squeeze(0)
                label_half1 = label_half1.float().squeeze(0)
                featV_half2 = featV_half2.squeeze(0)
                ...
                for b in range(int(np.ceil(len(featV_half1)/BS))):
                    start_frame = BS*b
                    ...
                    classV, classA, Vreal, Vpreds, Areal, Apreds, outputs = model(featV, featA)
